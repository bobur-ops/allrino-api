...
if (local) {
    ...
    newBase.password = oldBase.user = '991efa6945';
    } 
...
index.post('/admin/app', function (req, res) {
 ...
 let folderPath = path.join(__dirname, '../base/');
  fs.writeFileSync(folderPath + 'firms.json', req.body.str, "utf8"); 
   ...
   var tabFirma = 'spravka_catalog';
    ...
    promise.push(mysql.createConnection(oldBase).then(function (conn) {
           ...
           return true;
            ...
            if (connection && connection.end) connection.end();
            console.log(error);
                return res.send('Ошибка соеденения со старой базой2 ');
            }));
    ...
    promise.push(mysql.createConnection(oldBase).then(function (conn) {
        ...
        for (let i = 0; i < firms.length; i++) {
            if (firms[i].address === '' || firms[i].address === '-' || firms[i].address === '.') {
                multAdr.push([{ adres: '', long: null, latid: null, city: '', tel1: firms[i].phones, what1: false, dop1: firms[i].phone1_comment, tel2: firms[i].fax, what2: false, dop2: firms[i].phone2_comment, tel3: firms[i].dop_phones, what3: false, dop3: firms[i].phone3_comment }]);
                adr.push('');
            } else {
                multAdr.push([{ adres: firms[i].address, long: firms[i].longitude, latid: firms[i].latitude, city: 'Томск', tel1: firms[i].phones, what1: false, dop1: firms[i].phone1_comment, tel2: firms[i].fax, what2: false, dop2: firms[i].phone2_comment, tel3: firms[i].dop_phones, what3: false, dop3: firms[i].phone3_comment }]);
        ...
        for (let i = 0; i < adr.length; i++) {
            let ter = geocoder.geocode([adr[i]], { lang: 'en-US' })
                .then(function (res) {
    ...
    //Записываем их в новом формате
        ...
        for (let i = 0; i < FirmsAll.length; i++) {
            ...
            if (FirmsAll[i].picture !== null && FirmsAll[i].picture !== undefined && FirmsAll[i].picture !== '') {
                ...
                //fs.stat(pathAll, function (err, stats) {
                //    if (err) {
                //        console.log("Файл не найден = " + pathAll);
                //    } else {
                //        ban = 
                //        if (ban !== '' && ban !== null && ban !== undefined) {
                ...
                for (let key in firmBan.firmsId) {
                    if (firmBan.firmsLink[key] === FirmsAll[i].id + '') {
                        ...
                        let ban = fs.readFileSync(pathAll, "utf8");
                        let banRez = '<svg' + ban.split('<svg')[1];
                        //fs.stat(folderPath + '/' + firmBan.firmsBaner[key] + 'small.svg', function (err, stats) {
                        //    if (err) {
                        //        console.log("Файл не найден = " + firmBan.firmsBaner[key] + 'small.svg');
                        //    } else {
                        //        ban = fs.readFileSync(folderPath + '/' + firmBan.firmsBaner[key] + 'small.svg', "utf8");
                        //        if (ban !== '' && ban !== null && ban !== undefined) {
                        //            banRez = '<svg' + ban.split('<svg')[1];
                        //        }
                        //    }
                        //});
